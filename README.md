...
A dockerfile has been added to build the repo.

To build run: 

```docker build -t gaussian-splatting:cuda11.7-pytorch1.13-ubuntu22 . -f Dockerfile```

To run the container with gui capability run docker script located in scripts:
```
cd scripts
./run-docker.sh
```

Otherwise use the arguments contained in the script if the gui is not needed.
